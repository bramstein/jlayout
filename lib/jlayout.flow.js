...
(function () {
	...
	jLayout.OutOfBoundsError = function (dimension, containerSize) {
		this.dimension = dimension;
		this.containerSize = containerSize;
		this.message = ["Not all components fit into the container of", this.dimension, this.containerSize[this.dimension]].join(" ");
		this.name = "jLayout::OutOfBoundsError";
	};
	jLayout.OutOfBoundsError.prototype = new RangeError();
	jLayout.OutOfBoundsError.prototype.constructor = jLayout.OutOfBoundsError;
	...
	jLayout.flow = function (options) {
		...
		function align(row, offset, rowSize, parentSize) {
			...
			for (; i < len; i += 1) {
				var itemSize = row[i].bounds();
				if (location.x < 0 || location.x + itemSize.width > parentSize.width) {
					throw new jLayout.OutOfBoundsError("width", parentSize);
				}
				location.y = offset.y;
				if (location.y < 0 || location.y + itemSize.height > parentSize.height) {
					throw new jLayout.OutOfBoundsError("height", parentSize);
				}
