...
(function () {
	...
	jLayout.flow = function (options) {
		...
		that.OutOfBoundsError = function (dimension, containerDimension) {
			this.message = ["Not all components fit into the container of", dimension, containerDimension].join(" ");
			this.name = "jLayout::OutOfBoundsError";
		};
		that.OutOfBoundsError.prototype = new RangeError();
		that.OutOfBoundsError.prototype.constructor = that.OutOfBoundsError;
		...
		function align(row, offset, rowSize, parentSize) {
			...
			for (; i < len; i += 1) {
				if (location.x < 0 || location.x + row[i].width > parentSize.width) {
					throw new that.OutOfBoundsError("width", parentSize.width);
				}
				location.y = offset.y;
				if (location.y < 0 || location.y + row[i].height > parentSize.height) {
					throw new that.OutOfBoundsError("height", parentSize.height);
				}
